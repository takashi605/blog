Feature: 記事投稿
  ...
  Scenario: 記事を投稿する
    ...
    When 「世界」を選択し、インラインコードボタンを押す
    Then リッチテキストエディタに「こんにちは！世界」と表示され、世界がインラインコードになっている
    ...
    When 言語選択セレクトボックスから、「js」を選択
    Then コードブロックの言語データ属性が「js」になっている
    ...
    When 投稿した記事のページにアクセスする
    Then タイトルが「テスト記事」になっている
    Then 選択したサムネイル画像が表示されている
    Then 本文に「こんにちは！世界」と表示されている
    Then 世界が太字になっていない
    Then 世界がインラインコードになっている
    Then 「見出し2」という文字の h2 が存在する
    Then 「見出し3」という文字の h3 が存在する
    Then コードブロックが存在している
    Then コードブロックの言語が「js」になっている
    Then コードブロック内に「const a = 1」が表示されている
    Then コードのコピーボタンが表示されている
    Then 画像が表示されている
    Then 投稿日が今日の日付になっている
    Then 更新日が今日の日付になっている

    When 新着記事一覧ページにアクセスする
    Then 投稿した記事が新着記事一覧に表示されている
