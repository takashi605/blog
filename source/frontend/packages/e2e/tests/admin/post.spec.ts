...
Then('リッチテキストエディタが表示されていることを確認する', async function () {
  ...
  await expect(richTextEditor).toBeVisible({ timeout: 30000 });
});
...
When('リッチテキストエディタに「こんにちは！」と入力する', async function () {
  ...
  const richTextEditor = page.locator('[contenteditable="true"]');
  await richTextEditor.pressSequentially('こんにちは！', { timeout: 30000 });
});
Then('リッチテキストエディタに「こんにちは！」が表示される', async function () {
  const page = playwrightHelper.getPage();

  const richTextEditor = page.locator('[contenteditable="true"]');
  await expect(richTextEditor).toHaveText('こんにちは！', { timeout: 30000 });
});
When('「世界」と入力し、その文字を選択して太字ボタンを押す', async function () {
  ...
  const richTextEditor = page.locator('[contenteditable="true"]');
  // テキストをセット
  await richTextEditor.pressSequentially('世界', { timeout: 30000 });
...
Then(
  ...
  async function () {
    ...
    await expect(boldText).toHaveText('世界', { timeout: 30000 });
  },
);
...
Then(
  ...
  async function () {
    ...
    const boldText = richTextEditor.locator('strong');
    await expect(boldText).not.toBeVisible({ timeout: 30000 });
  },
);
When(
...
Then(
  ...
  async function () {
    ...
    await expect(h2Text).toHaveText('見出し2', { timeout: 30000 });
  },
);
When(
...
Then(
  ...
  async function () {
    ...
    await expect(h3Text).toHaveText('見出し3', { timeout: 30000 });
  },
);
...
//   await expect(page.getByRole('textbox', { name: 'h2' })).toBeVisible({timeout: 30000});
// });
...
//   await expect(page.getByRole('textbox', { name: 'paragraph' })).toBeVisible({timeout: 30000});
// });
...
//   await expect(page.getByRole('textbox', { name: 'h3' })).toBeVisible({timeout: 30000});
// });
...
//   await expect(page.getByText('記事を公開しました')).toBeVisible({timeout: 30000});
// });
