...
describe('エンティティ：投稿記事の日付', () => {
  it('日付を生成できる', () => {
    const blogPostDate = new BlogPostDate('2021-01-01');
  ...
  it('YYYY-MM-DD の形式ではない文字列を渡すと EntityError が発生する', () => {
    expect(() => new BlogPostDate('')).toThrow(
      '日付は YYYY-MM-DD 形式で指定してください',
    );
    expect(() => new BlogPostDate('2021-01-01 00:00:00')).toThrow(
      '日付は YYYY-MM-DD 形式で指定してください',
    );
    expect(() => new BlogPostDate('2021/01/01')).toThrow(
      '日付は YYYY-MM-DD 形式で指定してください',
    );
  });
