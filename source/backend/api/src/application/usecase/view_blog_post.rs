...
mod tests {
  ...
  async fn test_retrieves_published_blog_post_data_from_repository() {
    ...
    // 過去の日付で公開日を設定（公開済み状態）
    let past_date = JstDate::from_naive_date(NaiveDate::from_ymd_opt(2024, 1, 1).unwrap());
  ...
  async fn test_returns_error_when_trying_to_view_unpublished_post() {
    ...
    // 未来の日付で公開日を設定（未公開状態）
    let future_date = JstDate::from_naive_date(NaiveDate::from_ymd_opt(3000, 12, 31).unwrap());
